...
def work_entries(data, language, category=None):
...
def make_work(drafts, selector=None, limit=25, offset=0, language=None, category=None):
    ...
    if selector:
        ...
        candidates = [item for item in drafts if work_entries(item[1], language, category)]
        if not candidates:
            raise ValueError("Keine offenen Übersetzungen mehr vorhanden")
        path, draft = min(candidates, key=lambda item: (len(work_entries(item[1], language, category)),
                                                       str(item[1].get("mod_id") or item[1].get("name") or item[0].name).casefold()))
    opened = work_entries(draft, language, category)
...
def run(selector=None, limit=25, offset=0, language=None, category=None):
    work = make_work(load_drafts(), selector, limit, offset, language, category)
